...
@section('title', 'Edit Monitored Category')

@section('content')
<div class="container-fluid">
    <!-- Page Heading -->
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Edit Monitored Category</h1>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('sensible-categories.update', $sensibleCategory->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <select name="category_id" id="category_id" class="form-control select2 @error('category_id') is-invalid @enderror" required>
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ old('category_id', $sensibleCategory->category_id) == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" name="min_quantity" id="min_quantity" class="form-control @error('min_quantity') is-invalid @enderror" 
                               value="{{ old('min_quantity', $sensibleCategory->min_quantity) }}" min="1" placeholder="Enter minimum stock quantity" required>
                        @error('min_quantity')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="email" name="notification_email" id="notification_email" class="form-control @error('notification_email') is-invalid @enderror" 
                               value="{{ old('notification_email', $sensibleCategory->notification_email) }}" placeholder="Enter email address" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="notification_frequency" id="notification_frequency" class="form-control @error('notification_frequency') is-invalid @enderror" required>
                            ...
                            <option value="daily" {{ old('notification_frequency', $sensibleCategory->notification_frequency) == 'daily' ? 'selected' : '' }}>Daily</option>
                            <option value="weekly" {{ old('notification_frequency', $sensibleCategory->notification_frequency) == 'weekly' ? 'selected' : '' }}>Weekly</option>
                            <option value="monthly" {{ old('notification_frequency', $sensibleCategory->notification_frequency) == 'monthly' ? 'selected' : '' }}>Monthly</option>
                        </select>
                ...
                <div class="mb-3">
                    <div class="form-check">
                        <input type="checkbox" name="is_active" id="is_active" class="form-check-input" 
                               {{ old('is_active', $sensibleCategory->is_active) ? 'checked' : '' }} value="1">
