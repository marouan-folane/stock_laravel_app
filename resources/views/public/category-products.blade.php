...
<head>
    ...
    <title>{{ $category->name }} Products - {{ config('app.name', 'Stock Management System') }}</title>
...
<body>
    ...
    <section class="py-5">
        <div class="container">
            <div class="row mb-4">
                <div class="col-md-6">
                    <h1 class="fw-bold mb-0">{{ $category->name }} Products</h1>
                    <p class="text-muted">Showing {{ $products->count() }} of {{ $products->total() }} products</p>
                </div>
                <div class="col-md-6">
                    <form action="{{ route('public.category.products', $category->id) }}" method="GET" class="d-flex">
                        <input type="text" name="search" class="form-control me-2" placeholder="Search in {{ $category->name }}..." value="{{ request('search') }}">
            ...
            <div class="mb-4">
                ...
                <div class="d-flex flex-wrap">
                    <a href="/products/all" class="badge bg-secondary bg-opacity-10 text-dark category-filter">
                        All
                    </a>
                    @foreach($categories as $cat)
                        <a href="{{ route('public.category.products', $cat->id) }}" class="badge bg-secondary bg-opacity-10 text-dark category-filter {{ $category->id == $cat->id ? 'active' : '' }}">
                            {{ $cat->name }}
                        </a>
                    @endforeach
                </div>
                ...
                @empty
                    <div class="col-12">
                        <div class="alert alert-info">
                            No products found in this category. Try a different category or check back later.
                        </div>
                    </div>
                @endforelse
            </div>
